...
section_dir_tree <- function(title, roots, notes = list(), max_entries = 800) {
  ...
  if (length(notes)) {
    ...
    notes_html <- sprintf(
      "<div style='margin-top:6px'><strong>What you’ll find:</strong><ul>%s</ul></div>",
      paste(li, collapse = "")
    )
  }
...
if (is.na(sup_csv)) {
  ...
  if (!nrow(x)) {
    ...
    cat("### Summary statistics\n\n")
    cat(sprintf("**GOseq terms:** %d total, %d with **FDR ≤ 0.05**.\n\n", nrow(x), nrow(x_sig)))

    if (!nrow(x_sig)) {
      cat("**GOseq:** 0 terms with **FDR ≤ 0.05** (no significant GOseq enrichment for this contrast).\n\n")
    } else {
      stats_tbl <- x_sig %>%
...
if (!nrow(rr_tbl)) {
  ...
  sprintf("RRvGO Treemap — %s — %s", ont, m),
  sub$full_path[1]  # ✅ Use the absolute path from index
))
        }
      }
    }
...
grab_net <- function(mode_dir, ont) {
  ...
  if (any(matches)) idx$full_path[which(matches)[1]] else NA_character_  # ✅ Use full_path from index
}
...
embed_html_toggle_external_plus_local("Interactive Network — BP (with background)", wb_bp)
embed_html_toggle_external_plus_local("Interactive Network — CC (with background)", wb_cc)
embed_html_toggle_external_plus_local("Interactive Network — MF (with background)", wb_mf)
embed_html_toggle_external_plus_local("Interactive Network — BP (exploratory)",     we_bp)
embed_html_toggle_external_plus_local("Interactive Network — CC (exploratory)",     we_cc)
embed_html_toggle_external_plus_local("Interactive Network — MF (exploratory)",     we_mf)
