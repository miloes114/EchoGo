...
This is the original EchoGO workflow. It assumes you have already run a **Trinity → Trinotate → GOseq**
...
- `dge_<CONTRAST>.csv`   (DESeq2 results; required by the current input resolver)
- `<CONTRAST>.GOseq.enriched.tsv`  (GOseq enriched categories)
- `Trinotate_for_EchoGO.tsv` *or* `Trout_eggNOG_for_EchoGO.tsv` (Trinotate-like/EggNOG annotations)
...
If your input directory already contains a precomputed GOseq enrichment table and Trinotate-like/EggNOG
annotations (reference-based workflow), you can run EchoGO directly from that folder:
